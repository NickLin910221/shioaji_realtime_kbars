...
class Realtime_Contracts():
    ...
    def subscribe(self, contract, last_days = 0):
        if contract.__class__ is sj.contracts.Stock:
            for _contract_ in self.stk_Contracts:
                if contract.contract.code == _contract_.contract.code:
                    return
            ...
            for _contract_ in self.fop_Contracts:
                if contract.target_code == _contract_.contract.target_code or contract.code == _contract_.contract.code:
                    return
    ...
    def update(self, tick, type):
        if type == "stk":
            for _contract_ in self.stk_Contracts:
                if tick.code == _contract_.contract.code:
                    _contract_.update(tick)
        elif type == "fop":
            for _contract_ in self.fop_Contracts:
                if tick.code == _contract_.contract.target_code or tick.code == _contract_.contract.code:
                    _contract_.update(tick)
    ...
    def Kbars(self, contract, period):
        if contract.__class__ is sj.contracts.Stock:
            for _contract_ in self.stk_Contracts:
                if contract.contract.code == _contract_.contract.code:
                    return _contract_.getKlines(period)
            return data.Kbars(ts = [], Open = [], High = [], Low = [], Close = [], Volume = [], Amount = []) 
        elif contract.__class__ is sj.contracts.Future or contract.__class__ is sj.contracts.Option:
            for _contract_ in self.fop_Contracts:
                if contract.code == _contract_.contract.code:
                    return _contract_.getKlines(period)
            return data.Kbars(ts = [], Open = [], High = [], Low = [], Close = [], Volume = [], Amount = [])